            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                        this.sendButton(m.chat, `あなたのリミットわもういない, ketik *${usedPrefix}buy*`, wm, 'Buy', '.buy', m)
                    ...
                    try {
                        ...
                        if (e) {
                            ...
                            for (let jid of global.owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != this.user.jid)) {
                                ...
                                if (data.exists)
                                    m.reply(`*プラギン:* ${m.plugin}\n*送信者:* @${m.sender.split`@`[0]}\n*チャット:* ${m.chat}\n*チャットネーム:* ${await this.getName(m.chat)}\n*コマンド:* ${usedPrefix}${command} ${args.join(' ')}\n\n\`\`\`${text}\`\`\``.trim(), data.jid, { mentions: [m.sender] })
                            }
                            m.reply(text)
                        }
                    } finally {
                        ...
                        if (m.limit) m.reply(+ m.limit + '制限が使用されました.')
                ...
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        ...
                        try {
                            ...
                            await this.send3TemplateButtonImg(id, action === 'add' ? wel : lea, text, wm, action === 'add' ? 'Intro :v' : 'Dadah ngab :v', action === 'add' ? '.intro' : 'zowner')
                        }
                    }
                }
                break
